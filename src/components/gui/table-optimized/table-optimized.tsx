...
export default class OptimizeTableState {
  ...
  protected headers: DatabaseHeader[] = [];
  ...
  constructor(headers: DatabaseHeader[], data: Record<string, unknown>[]) {
  ...
  getSelectionAggregatedResult() {
    ...
    if (detectedDataType === "number") {
      return {
        sum: formatNumber(sum as number),
        avg: formatNumber(avg as number),

...
interface HotTablePaginationProps<TData> {
  table: Table<TData>;
  pageSizeOptions?: number[];
}

export function HotTablePagination<TData>({
  table,
  pageSizeOptions = [10, 20, 30, 40, 50, 100, 500],
}: HotTablePaginationProps<TData>) {
    ...
    <div className="flex w-full flex-col-reverse items-center justify-end gap-4 overflow-auto p-1 sm:flex-row sm:gap-8">
      <div className="flex flex-col-reverse items-center gap-4 sm:flex-row sm:gap-6 lg:gap-8">
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            <ChevronsLeft className="w-4 h-4 stroke-1" />
          </Button>
          <Button
            variant="outline"
            size="sm"
          ...
          <Button
            ...
            disabled={!table.getCanNextPage()}
          >
            <ChevronsRight className="w-4 h-4 stroke-1" />
          </Button>
        </div>
      </div>
    </div>
